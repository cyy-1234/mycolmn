...
import com.my.column.util.DeleteFileByUrl;
...
public class ArticleController {
    ...
    @ResponseBody
    public Result delete(HttpServletRequest request, Long articleId) {
//        System.out.println(request.getParameter("articleId"));
////        System.out.println(articleId);
//        System.out.println(article);
        int isdelete = articleService.deleteByPrimaryKey(articleId);
        if (isdelete>0) {
            System.out.println("文章删除成功");
            String oldpic=articleService.getArticleEntityById(articleId).getArticleCoverImage();
            new DeleteFileByUrl().delete(oldpic);
            return ResultGenerator.genSuccessResult();
        } else {
